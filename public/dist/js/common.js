...
jQuery(function () {
  $(document).ready(function () {
    ...
    */
    ...
    $(document).on('click', '.js-menu-btn', GnbMenu); //Gnb 메뉴 클릭시 공통액션
    ...
    */
    ...
    $(document).on('click', '.js-share-btn', detailShareOpen); //수량 카운트하기
    ...
    $(document).on('click', '.js-minus', quantityMinus); //제품 서브메뉴 스크롤 픽시드
    ...
    $(document).on('click', '.js-qna-more', qnaMore); //셀렉트 박스 텍스트 컬러변경(placeholder 시각적 효과)
...
jQuery(function () {
  ...
  ;
  $(document).on('change', '.js-custom-message', customOption); //애니메이션
  ...
  if ($('.a-Opacity').length > 0) {
    $('.a-Opacity').each(function () {
      var Opacity = new ScrollMagic.Scene({
        ...
        triggerHook: 0.9
      }).reverse(false).setClassToggle(this, 'is-Opacity').addTo(controller);
    });
  }
}); //입고알림 버튼클릭시
